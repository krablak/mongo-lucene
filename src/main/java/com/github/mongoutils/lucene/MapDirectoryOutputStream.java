...
public class MapDirectoryOutputStream extends IndexOutput implements Accountable {
	...
	/*
	 * public void writeTo(final IndexOutput out) throws IOException { flush();
	 * final long end = file.length; long pos = 0; int buffer = 0; while (pos <
	 * end) { int length = bufferSize; long nextPos = pos + length; if (nextPos
	 * > end) { length = (int) (end - pos); }
	 * out.writeBytes(file.getBuffer(buffer++), length); pos = nextPos; } dirty
	 * = true; }
	 * 
	 * public void writeTo(byte[] bytes, int offset) throws IOException {
	 * flush(); final long end = file.length; long pos = 0; int buffer = 0; int
	 * bytesUpto = offset; while (pos < end) { int length = bufferSize; long
	 * nextPos = pos + length; if (nextPos > end) { // at the last buffer length
	 * = (int) (end - pos); } System.arraycopy(file.getBuffer(buffer++), 0,
	 * bytes, bytesUpto, length); bytesUpto += length; pos = nextPos; } }
	 */
	public void reset() {
		currentBuffer = null;
		currentBufferIndex = -1;
	...
	/*
	 * public long sizeInBytes() { return file.numBuffers() * bufferSize; }
	 */
